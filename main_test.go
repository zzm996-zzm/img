...
func TestHandleIndexRendersImprovedMarkdownParser(t *testing.T) {
	...
	for _, expected := range []string{
		...
		`auto-render.min.js`,
		`highlight.min.js`,
		`class="markdown-preview output preview markdown-body"`,
		...
		`class="tool-panel markdown-tool"`,
		`class="markdown-workbench split"`,
		`data-style="Default"`,
		`value="Default"`,
		`value="Academic"`,
		`value="Minimal"`,
		`value="Custom"`,
		`id="markdownCustomPanel"`,
		`id="customFontSize" type="range" min="12" max="18"`,
		`id="customLineHeight" type="range" min="1.2" max="2"`,
		`id="customMargin" type="range" min="10" max="30"`,
		`id="customTitleColor" type="color"`,
		`id="customFontFamily"`,
		`.preview.markdown-body[data-style="Academic"]{font-family:Georgia,"Times New Roman",serif;line-height:1.8}`,
		`.preview.markdown-body[data-style="Minimal"]`,
		`.preview.markdown-body[data-style="Custom"]{font-family:var(--md-font-family);font-size:var(--md-font-size);line-height:var(--md-line-height)}`,
		`white-space:normal;margin-top:0`,
		`border-bottom:0`,
		`box-shadow:none`,
		`body.academic{font-family:Georgia,"Times New Roman",serif;line-height:1.8`,
		`body.minimal`,
		`body.custom{font-family:var(--md-font-family);font-size:var(--md-font-size);line-height:var(--md-line-height)`,
		`function customMarkdownSettings`,
		`function updateCustomMarkdownStyle`,
		`function customMarkdownStyleAttribute`,
		`custom:customMarkdownSettings()`,
		`id="markdownStats"`,
		`id="markdownDraftStatus"`,
		`onclick="saveMarkdownFile()"`,
		`onchange="setMarkdownViewMode(this.value)"`,
		`onchange="setMarkdownStyle(this.value)"`,
		`oninput="renderMarkdown(false)"`,
		`function loadMarkdownFile`,
		`function saveMarkdownDraft`,
		`function updateMarkdownStats`,
		`function setMarkdownStyle`,
		`function setMarkdownViewMode`,
		`function applyMarkdownEnhancements`,
		`function markdownDocumentTitle`,
		`localStorage.setItem(MARKDOWN_STORAGE_KEY`,
		`renderMathInElement`,
