...
<div class="gradient-design-controls">
<label>Card size<select id="gradientCardSize" onchange="updateGradient()"><option value="1600x900">16:9 landscape</option><option value="1200x630">Social preview</option><option value="1080x1080">Square post</option><option value="1080x1920">Story / reel</option></select></label>
<label>Shape<select id="gradientCardShape" onchange="updateGradient()"><option value="soft">Soft rounded</option><option value="sharp">Sharp rectangle</option><option value="round">Large rounded</option><option value="pill">Pill card</option></select></label>
<label>Text style<select id="gradientTextStyle" onchange="updateGradient()"><option value="light">Light text</option><option value="dark">Dark text</option><option value="glass">Glass panel</option><option value="none">No text</option></select></label>
</div>
<div class="gradient-text-controls">
<label>Title<input id="gradientTitle" value="Launch faster" oninput="updateGradient()"></label>
<label>Subtitle<input id="gradientSubtitle" value="A vivid gradient card for products, posts and announcements." oninput="updateGradient()"></label>
</div>
...
<div id="gradientCardContent" class="gradient-card-content"><strong id="gradientPreviewTitle"></strong><span id="gradientPreviewSubtitle"></span></div>
</div>
...
function gradientCardSize(){const value=document.getElementById('gradientCardSize').value.split('x').map(Number);return {width:value[0]||1600,height:value[1]||900};}
function updateGradient(){const css=gradientCSS();const preview=document.getElementById('gradientPreview');const shape=document.getElementById('gradientCardShape').value;const textStyle=document.getElementById('gradientTextStyle').value;preview.style.background=css;preview.className='gradient-preview shape-'+shape+' text-'+textStyle;document.getElementById('gradientPreviewTitle').textContent=document.getElementById('gradientTitle').value.trim();document.getElementById('gradientPreviewSubtitle').textContent=document.getElementById('gradientSubtitle').value.trim();document.getElementById('gradientCode').textContent='background: '+css+';';setGradientStatus('');}
...
function roundedRectPath(ctx,x,y,w,h,r){const radius=Math.min(r,w/2,h/2);ctx.beginPath();ctx.moveTo(x+radius,y);ctx.lineTo(x+w-radius,y);ctx.quadraticCurveTo(x+w,y,x+w,y+radius);ctx.lineTo(x+w,y+h-radius);ctx.quadraticCurveTo(x+w,y+h,x+w-radius,y+h);ctx.lineTo(x+radius,y+h);ctx.quadraticCurveTo(x,y+h,x,y+h-radius);ctx.lineTo(x,y+radius);ctx.quadraticCurveTo(x,y,x+radius,y);ctx.closePath();}
function gradientRadius(width,height){const shape=document.getElementById('gradientCardShape').value;if(shape==='sharp')return 0;if(shape==='round')return Math.min(width,height)*0.08;if(shape==='pill')return Math.min(width,height)/2;return Math.min(width,height)*0.035;}
function gradientCanvasPoints(width,height){const direction=document.getElementById('gradientDirection').value;if(direction==='90deg'||direction==='to right')return [0,0,width,0];if(direction==='180deg'||direction==='to bottom')return [0,0,0,height];if(direction==='45deg')return [0,height,width,0];return [0,0,width,height];}
function wrapGradientText(ctx,text,maxWidth){const words=text.trim().split(/\s+/).filter(Boolean);const lines=[];let line='';words.forEach(word=>{const test=line?line+' '+word:word;if(ctx.measureText(test).width>maxWidth&&line){lines.push(line);line=word;}else{line=test;}});if(line)lines.push(line);return lines.slice(0,3);}
function drawGradientCardText(ctx,width,height){const style=document.getElementById('gradientTextStyle').value;if(style==='none')return;const title=document.getElementById('gradientTitle').value.trim();const subtitle=document.getElementById('gradientSubtitle').value.trim();if(!title&&!subtitle)return;const pad=Math.round(Math.min(width,height)*0.08);const maxWidth=Math.round(width*0.72);let x=pad,y=height-pad;if(style==='glass'){const panelW=Math.min(Math.round(width*0.72),Math.max(460,Math.round(width*0.46)));const panelH=Math.round(Math.min(height*0.34,260));roundedRectPath(ctx,pad,height-pad-panelH,panelW,panelH,Math.round(panelH*0.12));ctx.fillStyle='rgba(255,255,255,0.18)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,0.3)';ctx.lineWidth=Math.max(2,width/800);ctx.stroke();x=pad+Math.round(panelH*0.1);y=height-pad-Math.round(panelH*0.18);}ctx.textBaseline='bottom';ctx.fillStyle=style==='dark'?'rgba(15,15,17,0.94)':'#ffffff';ctx.shadowColor=style==='dark'?'transparent':'rgba(0,0,0,0.26)';ctx.shadowBlur=Math.round(Math.min(width,height)*0.018);ctx.font='800 '+Math.round(Math.min(width,height)*0.08)+'px sans-serif';const titleLines=wrapGradientText(ctx,title,maxWidth);for(let i=titleLines.length-1;i>=0;i--){ctx.fillText(titleLines[i],x,y);y-=Math.round(Math.min(width,height)*0.09);}if(subtitle){ctx.shadowBlur=0;ctx.fillStyle=style==='dark'?'rgba(15,15,17,0.7)':'rgba(255,255,255,0.82)';ctx.font='400 '+Math.round(Math.min(width,height)*0.028)+'px sans-serif';const subLines=wrapGradientText(ctx,subtitle,maxWidth).slice(0,2);let subY=height-pad+Math.round(Math.min(width,height)*0.055);subLines.forEach(line=>{ctx.fillText(line,x,subY);subY+=Math.round(Math.min(width,height)*0.04);});}}
function downloadGradientPNG(){const size=gradientCardSize();const canvas=document.createElement('canvas');canvas.width=size.width;canvas.height=size.height;const ctx=canvas.getContext('2d');const points=gradientCanvasPoints(canvas.width,canvas.height);const grad=ctx.createLinearGradient(points[0],points[1],points[2],points[3]);gradientColors().forEach((color,index)=>grad.addColorStop(index/(gradientColors().length-1),color));roundedRectPath(ctx,0,0,canvas.width,canvas.height,gradientRadius(canvas.width,canvas.height));ctx.fillStyle=grad;ctx.fill();drawGradientCardText(ctx,canvas.width,canvas.height);canvas.toBlob(blob=>{if(!blob)return;const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='onlinebox-gradient-card.png';a.click();URL.revokeObjectURL(url);setGradientStatus('PNG downloaded at '+canvas.width+'x'+canvas.height+'.');},'image/png');}
...
.canvas-wrap canvas{max-width:100%;height:auto}.wide canvas{width:100%;max-width:520px}.gradient-tool{display:grid;gap:14px}.gradient-controls{display:grid;grid-template-columns:1.25fr repeat(3,minmax(92px,1fr));gap:12px;align-items:end}.gradient-design-controls,.gradient-text-controls{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.gradient-text-controls{grid-template-columns:1fr 1.5fr}.gradient-controls label,.gradient-design-controls label,.gradient-text-controls label{margin:0}.gradient-controls input[type=color]{height:48px;padding:5px;cursor:pointer}.gradient-actions{display:flex;gap:10px;flex-wrap:wrap}.gradient-preview{position:relative;height:260px;border-radius:18px;border:1px solid var(--border);box-shadow:inset 0 0 0 1px rgba(255,255,255,.04);transition:background .18s ease,border-radius .18s ease;overflow:hidden;display:flex;align-items:flex-end;padding:28px}.gradient-preview.shape-sharp{border-radius:4px}.gradient-preview.shape-soft{border-radius:18px}.gradient-preview.shape-round{border-radius:34px}.gradient-preview.shape-pill{border-radius:999px}.gradient-card-content{max-width:min(82%,520px);display:grid;gap:8px;color:#fff;text-shadow:0 2px 20px rgba(0,0,0,.24)}.gradient-card-content strong{font-family:'Syne',sans-serif;font-size:clamp(28px,5vw,52px);line-height:1.02;letter-spacing:0}.gradient-card-content span{font-size:15px;line-height:1.55;color:rgba(255,255,255,.82)}.gradient-preview.text-dark .gradient-card-content{color:#111;text-shadow:none}.gradient-preview.text-dark .gradient-card-content span{color:rgba(0,0,0,.68)}.gradient-preview.text-glass .gradient-card-content{padding:18px 20px;border:1px solid rgba(255,255,255,.24);border-radius:16px;background:rgba(255,255,255,.15);backdrop-filter:blur(10px)}.gradient-preview.text-none .gradient-card-content{display:none}.gradient-swatches{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.gradient-preset{border:1px solid var(--border);border-radius:10px;min-height:56px;cursor:pointer;padding:0;overflow:hidden;background:var(--surface2);transition:transform .16s,border-color .16s}.gradient-preset:hover,.gradient-preset:focus-visible{border-color:var(--accent);outline:0;transform:translateY(-1px)}.gradient-preset span{display:block;height:100%;min-height:54px}.gradient-code{min-height:90px}
...
@media(max-width:620px){.two,.related,.zip-options,.zip-analysis,.gradient-controls,.gradient-design-controls,.gradient-text-controls,.gradient-swatches{grid-template-columns:1fr}.wrap{padding-top:30px}.gradient-actions .compact{flex:1 1 100%}.gradient-preview{height:220px;padding:20px}.gradient-card-content{max-width:100%}.gradient-card-content strong{font-size:30px}.m3u8-input-row{grid-template-columns:1fr}.m3u8-play-btn{width:100%}.m3u8-controls{grid-template-columns:auto 1fr auto;gap:7px}.m3u8-progress{grid-column:1/-1;grid-row:1}#m3u8Current{display:none}.m3u8-volume{width:76px}.speed-group{grid-column:1/-1;justify-content:flex-start}.m3u8-time{min-width:36px}}
</style>
</head>
<body>
<main class="wrap">
